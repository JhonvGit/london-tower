...
module.exports = async (req, res) => {
  try {
    ...
    const session = await read(req, db); if (!session || req.method !== 'PATCH') return json(res, 401, { error:'Sessão inválida.' });
    ...
    await db.collection('users').updateOne({ id:session.id }, { $set:{ passwordHash:hashPassword(String(password)), mustChange:false, updatedAt:new Date() } });
