(function(){
  ...
  bookingForm.onsubmit=e=>{e.preventDefault();state.date=date.value;if(!state.date||state.date<tomorrow()){show('Não é possível reservar hoje ou em datas anteriores. Escolha uma data futura.');return}if(!name.value.trim()||!apartment.value.trim()||!phone.value.trim()){show('Preencha nome, apartamento e telefone.');return}termsModal.classList.remove('hidden')};
