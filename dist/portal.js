...
(() => {
  ...
  function renderRooms() {
    $('rooms').innerHTML=rooms.map(r=>`<button type="button" class="room ${room===r.name?'selected':''}" aria-pressed="${room===r.name}" data-room="${r.name}"><img class="room-img" src="${escape(r.image)}" alt="Imagem ilustrativa do salão ${r.name}"><div class="room-body"><div class="room-name">${r.name}</div><div class="room-desc">${r.desc}</div><div class="room-meta"><span>Salão de festas</span><span>até ${r.capacity} pessoas</span></div></div></button>`).join('');
  ...
  async function loadSettings() { const data=await api('settings');term=data.term;$('termEditor').value=term;document.querySelector('#termsModal .terms').textContent=term;if(Array.isArray(data.rooms)&&data.rooms.length){rooms=data.rooms;}renderRooms();renderAdminRooms(); }
  ...
  async function enter() {
    ...
    try { await Promise.all([loadBookings(),loadSettings()]); } catch(error) { show(error.message+' Use Atualizar para tentar novamente.'); }
  }
  ...
  $('navAdmin').onclick=async()=>{if(user?.role!=='admin')return;page('admin');try{await Promise.all([loadAccounts(),loadBookings(),loadSettings()]);}catch(error){show(error.message);}};
  $('refresh').onclick=async()=>{try{await Promise.all([loadBookings(),loadSettings()]);show('Dados atualizados.');}catch(error){show(error.message);}};
  ...
  $('saveTerm').onclick=async()=>{const button=$('saveTerm');button.disabled=true;try{await send('settings','PATCH',{term:$('termEditor').value});await loadSettings();show('Termo atualizado.');}catch(error){show(error.message);}finally{button.disabled=false;}};
  $('saveRooms').onclick=async()=>{const button=$('saveRooms');button.disabled=true;try{const updated=rooms.map(r=>{const capInp=$('adminRooms').querySelector(`input[data-room="${r.name}"][data-field="capacity"]`);const imgInp=$('adminRooms').querySelector(`input[data-room="${r.name}"][data-field="image"]`);return{name:r.name,capacity:Number(capInp?.value||r.capacity),image:imgInp?.value.trim()||r.image};});await send('settings','PATCH',{rooms:updated});await loadSettings();show('Salões atualizados com sucesso.');}catch(error){show(error.message);}finally{button.disabled=false;}};
  function renderAdminRooms(){$('adminRooms').innerHTML=rooms.map(r=>`<div class="account" style="flex-direction:column;align-items:stretch;gap:10px;padding:16px 0"><div style="display:flex;align-items:center;justify-content:space-between"><b>${r.name}</b><span class="badge">Até ${r.capacity} pessoas</span></div><label style="font-size:13px;font-weight:700;margin-bottom:4px">Capacidade máxima (pessoas)<input type="number" min="1" max="500" value="${r.capacity}" data-room="${r.name}" data-field="capacity" style="margin-top:6px;padding:8px 10px;border:1px solid var(--line);border-radius:8px;width:100%"></label><label style="font-size:13px;font-weight:700;margin-bottom:4px">URL da foto<input type="url" value="${escape(r.image)}" data-room="${r.name}" data-field="image" placeholder="https://..." style="margin-top:6px;padding:8px 10px;border:1px solid var(--line);border-radius:8px;width:100%"></label><img src="${escape(r.image)}" alt="Preview ${r.name}" style="width:100%;height:120px;object-fit:cover;border-radius:8px;border:1px solid var(--line)" onerror="this.style.display='none'" onload="this.style.display=''"></div>`).join('');
  $('adminRooms').querySelectorAll('input[data-field="image"]').forEach(inp=>inp.oninput=()=>{const prev=inp.closest('.account').querySelector('img');if(prev){prev.src=inp.value;prev.style.display='';}});
  }
  let currentReportData = null;
  function downloadCSV(month, bookings) {
    const monthLabel = new Intl.DateTimeFormat('pt-BR',{month:'long',year:'numeric'}).format(new Date(month+'-15'));
    const header = 'Data,Dia da Semana,Salão,Responsável,Apartamento,Telefone,Evento,Horário\n';
    const rows = bookings.map(b => {
      const dateObj = new Date(b.date + 'T12:00:00Z');
      const weekday = new Intl.DateTimeFormat('pt-BR', { weekday: 'long', timeZone: 'America/Sao_Paulo' }).format(dateObj);
      return `"${format(b.date)}","${weekday}","${b.room}","${b.name}","${b.apartment}","${b.phone}","${b.event}","${b.start} às ${b.end}"`;
    }).join('\n');
    const csv = '\uFEFF' + header + rows;
    const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' });
    const link = document.createElement('a');
    link.href = URL.createObjectURL(blob);
    link.download = `reservas_${month}.csv`;
    link.click();
    URL.revokeObjectURL(link.href);
  }
  function downloadPDF(month, bookings, stats) {
    const monthLabel = new Intl.DateTimeFormat('pt-BR',{month:'long',year:'numeric'}).format(new Date(month+'-15'));
    const win = window.open('', '_blank');
    win.document.write(`<!DOCTYPE html><html><head><meta charset="utf-8"><title>Relatório de Reservas - ${monthLabel}</title><style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:system-ui,-apple-system,sans-serif;padding:40px;max-width:1000px;margin:0 auto;background:#fff;color:#000}h1{font-size:24px;margin-bottom:8px}h2{font-size:18px;margin:24px 0 12px;border-bottom:2px solid #333;padding-bottom:4px}.meta{color:#666;font-size:14px;margin-bottom:24px}.stats{display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:12px;margin-bottom:24px}.stat-card{border:1px solid #ddd;padding:12px;border-radius:4px;text-align:center}.stat-value{font-size:32px;font-weight:bold;color:#0066cc}.stat-label{font-size:14px;color:#666;margin-top:4px}table{width:100%;border-collapse:collapse;margin-top:12px}th,td{padding:10px;text-align:left;border-bottom:1px solid #ddd}th{background:#f5f5f5;font-weight:600;font-size:13px;text-transform:uppercase;color:#333}td{font-size:14px}tr:hover{background:#f9f9f9}.room-oxford{border-left:3px solid #0066cc}.room-napoli{border-left:3px solid #00aa66}.room-rooftop{border-left:3px solid #cc6600}.footer{margin-top:40px;padding-top:20px;border-top:1px solid #ddd;font-size:12px;color:#666;text-align:center}@media print{body{padding:20px}@page{margin:1.5cm}}</style></head><body><h1>Relatório de Reservas de Salões de Festas</h1><div class="meta">London Tower Condomínio · ${monthLabel}</div><h2>Estatísticas do Período</h2><div class="stats"><div class="stat-card"><div class="stat-value">${stats.total}</div><div class="stat-label">Total de Reservas</div></div>`);
    Object.entries(stats.byRoom).forEach(([room, count]) => {
      win.document.write(`<div class="stat-card"><div class="stat-value">${count}</div><div class="stat-label">${room}</div></div>`);
    });
    win.document.write(`</div><h2>Detalhamento das Reservas</h2><table><thead><tr><th>Data</th><th>Dia</th><th>Salão</th><th>Responsável</th><th>Apto</th><th>Telefone</th><th>Evento</th><th>Horário</th></tr></thead><tbody>`);
    bookings.forEach(b => {
      const dateObj = new Date(b.date + 'T12:00:00Z');
      const weekday = new Intl.DateTimeFormat('pt-BR', { weekday: 'short', timeZone: 'America/Sao_Paulo' }).format(dateObj);
      const roomClass = `room-${b.room.toLowerCase()}`;
      win.document.write(`<tr class="${roomClass}"><td>${format(b.date)}</td><td>${weekday}</td><td><strong>${b.room}</strong></td><td>${b.name}</td><td>${b.apartment}</td><td>${b.phone}</td><td>${b.event}</td><td>${b.start}–${b.end}</td></tr>`);
    });
    win.document.write(`</tbody></table><div class="footer">Documento gerado em ${new Date().toLocaleString('pt-BR', {timeZone:'America/Sao_Paulo'})} · London Tower</div></body></html>`);
    win.document.close();
    setTimeout(() => win.print(), 500);
  }
  $('generateReport').onclick=async()=>{const monthValue=$('reportMonth').value;if(!monthValue){show('Selecione um mês para gerar o relatório.');return;}const button=$('generateReport');button.disabled=true;try{const data=await api(`reports?month=${monthValue}`);currentReportData=data;const monthLabel=new Intl.DateTimeFormat('pt-BR',{month:'long',year:'numeric'}).format(new Date(monthValue+'-15'));$('reportStats').innerHTML=`<div style="padding:8px;background:var(--bg);border-radius:4px"><strong style="display:block;font-size:20px;color:var(--primary)">${data.stats.total}</strong><span style="font-size:14px;color:var(--muted)">Total de reservas</span></div>`+Object.entries(data.stats.byRoom).map(([room,count])=>`<div style="padding:8px;background:var(--bg);border-radius:4px"><strong style="display:block;font-size:20px;color:var(--primary)">${count}</strong><span style="font-size:14px;color:var(--muted)">${escape(room)}</span></div>`).join('');$('reportBookings').innerHTML=data.bookings.length?`<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px"><h4 style="margin:0">Reservas de ${monthLabel}</h4><div style="display:flex;gap:8px"><button type="button" id="exportPDF" class="secondary" style="padding:6px 12px">📄 Exportar PDF</button><button type="button" id="exportCSV" class="secondary" style="padding:6px 12px">📊 Exportar CSV</button></div></div>`+data.bookings.map(b=>`<div class="admin-reservation"><div style="display:flex;justify-content:space-between;align-items:center"><b>${escape(b.room)} · ${escape(format(b.date))}</b><span class="badge">${escape(b.event)}</span></div><div style="margin-top:6px;font-size:14px"><strong>Responsável:</strong> ${escape(b.name)}<br><strong>Apartamento:</strong> ${escape(b.apartment)} &nbsp;|&nbsp; <strong>Telefone:</strong> ${escape(b.phone)}</div></div>`).join(''):`<p style="color:var(--muted);text-align:center;padding:20px">Nenhuma reserva registrada em ${monthLabel}.</p>`;if(data.bookings.length){$('exportPDF').onclick=()=>downloadPDF(monthValue,data.bookings,data.stats);$('exportCSV').onclick=()=>downloadCSV(monthValue,data.bookings);}$('reportResult').style.display='block';show('Relatório gerado com sucesso.');}catch(error){show(error.message);}finally{button.disabled=false;}};
